...
해시를 게시하는 것만으로는 부족하다 — 비전문가는 SmartScreen 의 '알 수 없는
게시자' 경고를 UI 를 보기도 전에 만나 설치를 포기한다. 설치본이 서명되기 전까지는
그 사용자가 **안전하게 통과할 경로**를 릴리스 노트가 직접 제공해야 완화가 실효를
갖는다(앱의 `denied` 오류 힌트도 이 대조를 전제로 안내한다). 아래를 릴리스마다
붙여넣고 `<버전>`·`<게시된 SHA256>` 만 채운다.

```markdown
### 설치 시 보안 경고에 대하여

이 버전은 아직 코드 서명 인증서가 없어, 처음 실행할 때 Windows SmartScreen 이
'알 수 없는 게시자' 경고를 띄웁니다. **정상입니다.** 다만 받은 파일이 배포자가
만든 것과 같은지 먼저 확인하십시오.

1. 내려받은 파일에서 PowerShell 로 해시를 계산합니다.
   `Get-FileHash .\Discan_<버전>_x64-setup.exe -Algorithm SHA256`
2. 아래 값과 **한 글자도 빠짐없이** 같은지 확인합니다.
   `SHA256: <게시된 SHA256>`
3. 값이 다르면 설치하지 말고 폐기하십시오(전송 중 손상 또는 위조).
4. 값이 같으면 경고 창의 '추가 정보 → 실행'으로 설치합니다.

설치 후의 자동 업데이트는 이 수동 대조가 필요 없습니다 — 업데이트 패키지는
개발자 개인키(minisign)로 서명되고 앱이 공개키로 검증하므로, 위조된 패키지는
설치 단계에서 거부됩니다. 수동 해시 대조는 **최초 설치 한 번**에만 해당합니다.
```

`createUpdaterArtifacts` 가 만드는 `.sig` 는 자동 업데이트 채널 전용이라 최초
설치에는 관여하지 않는다. 그래서 첫 설치의 진위는 Authenticode 서명이 붙기
전까지 이 SHA256 대조가 유일한 축이다 — 인증서를 확보하면 이 절이 통째로 불필요해진다.
