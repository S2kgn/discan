...
export interface ScanNode {
  ...
  lossyPath?: boolean;
  /**
   * 사용자가 용량 계산에서 제외한 노드(프런트 전용 표식, 백엔드엔 없다).
   *
   * 제외하면 그 서브트리 크기가 상위·총계에서 빠진다(applyFolderExclusions). 노드
   * 자신은 트리에 그대로 남아 원래 크기를 취소선으로 보여 주고 '원복'을 제공한다 —
 ...
 *
 * 일상어를 먼저 열고 개발 특유의 예시(빌드 폴더·.git 이력·가상환경)는 괄호 뒤로
 * 내린다 — 이 분야는 C: 등 일반 PC에서도 가장 큰 비중을 차지하는데, 비개발자에게
 * '빌드 산출물'·'가상환경'은 뜻 없는 문자열이라 지워도 되는지 판단에 도움이 되지 않는다.
 */
export const CATEGORY_WARNINGS: Record<string, string> = {
  cache:
    "브라우저·앱·시스템이 자동으로 다시 만드는 임시 폴더가 함께 잡힙니다. 캐시라고 모두 " +
    "지워도 되는 것은 아닙니다 — 프로그램을 개발하는 데 쓰는 폴더(빌드 폴더·.git 이력·" +
    "가상환경 등)가 섞여 있으면 지우기 전에 내용을 확인하십시오.",
};
