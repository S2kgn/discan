...
export function basisLines(result: ScanResult): string[] {
  ...
  // '930GB 디스크가 왜 866GiB인가'의 안심 문구(GiB = 탐색기 GB)는 지금까지 총 용량
  // 타일의 title 툴팁에만 있어 터치·키보드·낭독에서는 닿지 못했다. 키보드로 펼칠 수
  // 있는 이 집계 조건에 함께 실어, 마우스 호버 외의 경로에서도 도달하게 한다.
  lines.push(
    `집계 기준: ${basis} · ${dedup} · 온라인 전용 파일 제외 · 클러스터 슬랙 미반영 · ` +
      `용량 표기 GiB(1 KiB = 1024 B) — 탐색기가 'GB'로 적는 값과 같은 계산입니다`,
  );
